      ...
      private
        ...
        def write_asset_file_contents(joined_asset_path, asset_paths)
          ...
          begin
			...
			buffer = buffer.gsub(/@import url\("wow\.css"\);/, "")
            File.open(joined_asset_path, "w+") do |cache|
              cache.write(buffer)
            end
			# Using the highest modified time has issues if you either aren't touching the files often, or you're using remote files
			# Basically, the highest local file can have a date of March 20th, but it's actually March 25th, thus it'll try and pull down
			# Every file including the remotes until you retouch the local file
            # Set mtime to the latest of the combined files to allow for
            # consistent ETag without a shared filesystem.
            mt = asset_paths.reject {|p| p =~ %r{^[-a-z]+://} }.map { |p| File.mtime(asset_file_path(p)) }.max
            File.utime(mt, mt, joined_asset_path)
     		#now = Time.now
        	#File.utime(now, now, joined_asset_path)
